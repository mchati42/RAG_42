...
def find_markdown_sections(
    text: str,
) -> list[tuple[str, int, int]]:
    """Find Markdown sections with character offsets."""
    pattern = r"^#{1,6} .+$"
...
def chunk_markdown(
    ...
    """Split Markdown sections into chunks with absolute offsets."""
    chunks: list[tuple[str, int, int]] = []
    ...
    for section, start, end in sections:
        if len(section) <= chunk_size:
            ...
            section_chunks = split_source_chunk(
                section,
                start,
                chunk_size,
            )
